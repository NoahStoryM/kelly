...
(define string->seconds (λ (str #:format-string [fmt-str "~Y-~m-~d"]) (date->seconds (string->date str fmt-str))))
...
(define seconds->string
  (λ (secs-n [time? #f] [fmt 'iso-8601])
    (parameterize ([date-display-format fmt])
      (date->string (seconds->date secs-n) time?))))

(: string->milliseconds (-> String Integer))
(define string->milliseconds (λ (str) (* 1000 (string->seconds str))))
      ...
      (match datum
        [`(,ts ,o ,h ,l ,c ,vol ,volCcy ,volCcyQuote ,confirm)
         ...
         (let ([ts          (string->jsexpr-number ts)]
               ...
               [date        (seconds->string (/ (assert (string->number ts) real?) 1000) #t 'rfc2822)])
           `(,ts ,o ,h ,l ,c ,vol ,volCcy ,volCcyQuote ,confirm ,date))]))))
...
(: download (-> String Symbol Symbol String Natural Natural Natural [#:init-data (Listof JSExpr)] JSExpr))
(define download
  (let ()
    ...
    (λ (base-url name $ bar before after limit #:init-data [init-data '()])
      ...
      (define data
        (for/fold ([data : (Listof JSExpr) init-data])
                  ([current : Natural (in-range before after step)])
          (define make-url
            (let ([before current]
...
(define download-data
  (λ (inst-id before after #:history? [history? #t] #:write-file? [write-file? #t])
    ...
    (define timestamp (seconds->string (/ after 1000)))
...
(define (download-daily-data inst-id day #:history? [history? #t] #:write-file? [write-file? #t])
  (: before Natural)
  (define before (assert (sub1 (string->milliseconds day)) natural?))
  (: after Natural)
  (define after
    (if (today? day)
        (assert (current-milliseconds) natural?)
...
(: cache (Mutable-HashTable (Immutable-Vector Natural (Pair Symbol Symbol) String) (Listof Nonnegative-Real)))
...
(: cache->r** (-> (Pair Symbol Symbol) Natural (Listof String) (Listof (Listof Nonnegative-Real))))
(define cache->r**
  (λ (inst-id bar day*)
    (for/list : (Listof (Listof Nonnegative-Real))
              ([day : String (in-list day*)])
